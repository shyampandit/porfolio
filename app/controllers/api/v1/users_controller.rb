	...
	def register
		   ...
		   begin
		   	     # Sanitize POST data
					  ...
					  permitted_parameters = parameters.permit(:email,:firstname,:lastname, :password, :password_confirmation)
					  ...
					  if @user.save!
					        ...
					        render :json => { success: true, message: "success", data:UserSerializer.new(@user) }, :status => 201 
					 	end
				rescue Exception => e      # rescues all exceptions
						# Error Response
		        render :json => { success: false, 
		                          message: e.is_a?(ActiveRecord::RecordInvalid) ? (params[:lang_code] == "en") ? e.record.errors.full_messages[0] : e.record.errors.messages.values[0].join('') : e.message,
		                          data: {} }, :status => 400
